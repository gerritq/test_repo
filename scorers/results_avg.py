...
def main():
    ...
    languages = ['en', 'pt', 'vi']
    # Add Italian to display names but not to languages list to keep it empty
    display_languages = languages.copy()  # Original languages for data processing
    display_languages_extended = languages.copy() + ['it']  # Extended list for display
    ...
    for dir_name, task_display, subtasks in task_info:
        ...
        for task_prefix, modifier in subtasks:
            ...
            for lang in languages:  # Use original languages for file loading
    ...
    for task_key in set(k[0] for k in results_data.keys()):
        for lang in languages:  # Use original languages for processing
    ...
    generate_single_large_table(task_info, languages, display_languages_extended, language_display_names, 
                               detector_display_names, detector_families, 
                               avg_results, family_avg_results, output_file)

def generate_single_large_table(task_info, languages, display_languages_extended, language_display_names, 
                               detector_display_names, detector_families, 
                               avg_results, family_avg_results, output_file):
    ...
    # Updated column specification for the paired table with Italian columns
    # 1 for detector column + (4*2) for first task languages + 1 dummy column + (4*2) for second task languages
    col_spec = "l" + "cc"*4 + "c" + "cc"*4
    ...
    for pair_idx, task_pair in enumerate(task_pairs):
        ...
        for subtask_idx, (task1_prefix, modifier1) in enumerate(subtasks1):
            ...
            # Determine if we should add Italian columns for these tasks
            add_italian_for_task1 = task1_display == "Paragraph Continuation"
            add_italian_for_task2 = has_second_task and task2_display == "Text Style Transfer"
            
            # Add task headers row - only add detector name in first panel
            if pair_idx == 0:
                task_header = [""]
                # Adjust column span for task1 (8 columns including Italian for Paragraph Continuation)
                col_span_task1 = 8 if add_italian_for_task1 else 6
                task_header.extend([f"\\multicolumn{{{col_span_task1}}}{{c}}{{\\textbf{{{task1_display}}}}}"]) 
            ...
            if task2_key:
                # Adjust column span for task2 (8 columns including Italian for TST)
                col_span_task2 = 8 if add_italian_for_task2 else 6
                task_header.extend([f"\\multicolumn{{{col_span_task2}}}{{c}}{{\\textbf{{{task2_display}}}}}"]) 
            
            latex_code.append(" & ".join(task_header) + " \\\\")
            
            # Adjust cmidrule for task headers
            if add_italian_for_task1:
                cmidrule1 = "\\cmidrule(lr){2-9}"
            else:
                cmidrule1 = "\\cmidrule(lr){2-7}"
                
            if task2_key:
                if add_italian_for_task2:
                    # If Italian is included for task2, adjust the cmidrule
                    col_start = 11 if add_italian_for_task1 else 9
                    col_end = col_start + 7
                    cmidrule2 = f" \\cmidrule(lr){{{col_start}-{col_end}}}"
                else:
                    # Regular 6 columns for task2 (without Italian)
                    col_start = 9 if add_italian_for_task1 else 9
                    col_end = col_start + 5
                    cmidrule2 = f" \\cmidrule(lr){{{col_start}-{col_end}}}"
            else:
                cmidrule2 = ""
                
            latex_code.append(cmidrule1 + cmidrule2)
            ...
            # Use display languages for task1 - add Italian for Paragraph Continuation
            langs_for_task1 = display_languages_extended if add_italian_for_task1 else languages
            for lang in langs_for_task1:
            ...
            if task2_key:
                # Use display languages for task2 - add Italian for Text Style Transfer
                langs_for_task2 = display_languages_extended if add_italian_for_task2 else languages
                for lang in langs_for_task2:
                    header_row.append(f"\\multicolumn{{2}}{{c}}{{\\textbf{{{language_display_names[lang]}}}}}")
            ...
            # Add cmidrules for task1 languages
            langs_for_task1 = display_languages_extended if add_italian_for_task1 else languages
            for _ in range(len(langs_for_task1)):
            ...
            # Add cmidrules for task2 languages if it exists
            if task2_key:
                langs_for_task2 = display_languages_extended if add_italian_for_task2 else languages
                for _ in range(len(langs_for_task2)):
                    cmidrules.append(f"\\cmidrule(lr){{{col}-{col+1}}}")
                    col += 2
            ...
            # Add metrics headers for task1 languages
            langs_for_task1 = display_languages_extended if add_italian_for_task1 else languages
            for _ in range(len(langs_for_task1)):
            ...
            # Add metrics headers for task2 languages if it exists
            if task2_key:
                langs_for_task2 = display_languages_extended if add_italian_for_task2 else languages
                for _ in range(len(langs_for_task2)):
                    metrics_row.extend(["ACC", "F1"])
            ...
            for family_idx, (family, detectors) in enumerate(detectors_by_family.items()):
                ...
                for detector_key, detector_name in detectors:
                    ...
                    langs_for_task1 = display_languages_extended if add_italian_for_task1 else languages
                    for lang in langs_for_task1:
                        # Only process metrics for original languages with data
                        if lang in languages:
                            ...
                            # For Italian (or any additional languages), add empty cells
                            row.append("")  # Empty ACC
                            row.append("")  # Empty F1
                ...
                # Adjust cdashline spans for tables with Italian
                if add_italian_for_task1:
                    latex_code.append(f"\\cdashline{{1-9}}")  # First task with Italian
                else:
                    latex_code.append(f"\\cdashline{{1-7}}")  # First task without Italian
                
                if task2_key:
                    if add_italian_for_task2:
                        col_start = 11 if add_italian_for_task1 else 9
                        col_end = col_start + 7
                        latex_code.append(f"\\cdashline{{{col_start}-{col_end}}}")  # Second task with Italian
                    else:
                        col_start = 9 if add_italian_for_task1 else 9
                        col_end = col_start + 5
                        latex_code.append(f"\\cdashline{{{col_start}-{col_end}}}")  # Second task without Italian
                ...
                for lang in langs_for_task1:
                    ...
                    if lang in languages:
                        ...
                        # For Italian (or any additional languages), add empty cells
                        avg_row.append("")  # Empty ACC
                        avg_row.append("")  # Empty F1
