...
def main():
        ...
        for task_prefix, modifier in subtasks:
            ...
            for lang in languages:
                ...
                for model in generator_models:
                    ...
                    # Handle zero-shot results
                    if dir_name == 'sums':
                        # Special case for sums
                        if lang in ['pt', 'vi']:
                            zero_file = os.path.join(detect_dir, f"{lang}_sums_mgt_{modifier}_{model_id}_zero.jsonl")
                        else:
                            zero_file = os.path.join(detect_dir, f"{lang}_sums_mgt_{modifier}_{model_id}_zero.jsonl")
                    elif dir_name == 'neutral_new':
                        if lang in ['pt', 'vi']:
                            # No "default" in path for PT and VI in Text Style Transfer
                            zero_file = os.path.join(detect_dir, f"{lang}_mgt_{modifier}_{model_id}_zero.jsonl")
                        else:
                            zero_file = os.path.join(detect_dir, f"{lang}_{task_prefix}_mgt_{modifier}_{model_id}_zero.jsonl")
                    elif dir_name == 'paras':
                        zero_file = os.path.join(detect_dir, f"{lang}_paras_{modifier}_{task_prefix}_{model_id}_zero.jsonl")
                    
                    if os.path.exists(zero_file):
                        for detector in ['binoculars', 'llr', 'fastdetectgpt_white', 
                                        'revisedetect', 'gecscore', 'fastdetectgpt_black']:
                            key = (task_key, lang, model, f'zero_{detector}')
                            results_files[key] = zero_file
                    else:
                        print(f"File not found: {zero_file}")
                    
                    # Handle trained results
                    if dir_name == 'sums':
                        # Special case for sums
                        if lang in ['pt', 'vi']:
                            train_file = os.path.join(detect_dir, f"{lang}_sums_mgt_{modifier}_{model_id}_train.jsonl")
                        else:
                            train_file = os.path.join(detect_dir, f"{lang}_sums_mgt_{modifier}_{model_id}_train.jsonl")
                    elif dir_name == 'neutral_new':
                        if lang in ['pt', 'vi']:
                            # No "default" in path for PT and VI in Text Style Transfer
                            train_file = os.path.join(detect_dir, f"{lang}_mgt_{modifier}_{model_id}_train.jsonl")
                        else:
                            train_file = os.path.join(detect_dir, f"{lang}_{task_prefix}_mgt_{modifier}_{model_id}_train.jsonl")
                    elif dir_name == 'paras':
                        train_file = os.path.join(detect_dir, f"{lang}_paras_{modifier}_{task_prefix}_{model_id}_train.jsonl")
                    
                    if os.path.exists(train_file):
                        for detector in ['mdeberta', 'roberta']:
                            key = (task_key, lang, model, f'train_{detector}')
                            results_files[key] = train_file
                    else:
    ...
    for key, file_path in results_files.items():
        ...
        try:
            if detector.startswith('zero_'):
                detector_key = detector[5:]  # Remove 'zero_' prefix
                with open(file_path, 'r') as f:
                    data = json.load(f)
                    if detector_key in data:
                        accuracy = data[detector_key].get('accuracy')
                        f1 = data[detector_key].get('f1')
                        results_data[key] = (accuracy, f1)
            else:  # train
                detector_key = detector[6:]  # Remove 'train_' prefix
                with open(file_path, 'r') as f:
                    lines = f.readlines()
                    if lines:
    ...
    for dir_name, task_display, subtasks in task_info:
        for task_prefix, modifier in subtasks:
            ...
            for family, detectors in detectors_by_family.items():
                ...
                latex_code.append("\\cdashline{2-34}")
